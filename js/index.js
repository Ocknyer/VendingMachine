...
btnDeposit.addEventListener("click", (event) => {
    ...
    if (inputCost) {
        if (inputCost <= myMoney && inputCost > 0) {
            inhand.textContent =
                new Intl.NumberFormat().format(myMoney - inputCost) + "원";
            txtBalance.textContent =
                new Intl.NumberFormat().format(
                    (balance ? balance : 0) + inputCost
                ) + "원";
        } else {
            alert("소지금이 부족합니다.");
        }
...
btnReturn.addEventListener("click", (event) => {
    ...
    if (balance) {
        inhand.textContent = (balance + myMoney).toLocaleString() + "원";
...
btnCola.forEach((item) => {
    item.addEventListener("click", (event) => {
        ...
        if (balance >= itemPrice) {
            txtBalance.textContent =
                new Intl.NumberFormat().format(balance - itemPrice) + "원";
            ...
            if (!parseInt(targetEl.dataset.stock)) {
                ...
                targetEl.insertAdjacentHTML(
                    "afterbegin",
                    '<em class="txt-hide">해당상품은 품절입니다.</em>'
                );
            }
...
btnGet.addEventListener("click", () => {
        ...
        for (const itemGot of getItemList.querySelectorAll("li")) {
            ...
            if (itemStaged.dataset.item === itemGot.dataset.item) {
                itemGotCount.textContent =
                    ~~itemGotCount.textContent +
                    ~~itemStaged.querySelector(".num-counter").textContent;
                isGot = true;
                break;
            }
        }
    ...
    getItemList.querySelectorAll("li").forEach((itemGot) => {
        totalPrice +=
            ...
            ~~itemGot.querySelector(".num-counter").textContent;
    });
    total.textContent = `총금액 : ${new Intl.NumberFormat().format(
        totalPrice
    )} ₩`;
});
